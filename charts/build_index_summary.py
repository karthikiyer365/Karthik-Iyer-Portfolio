...
NOTEBOOK_PATH = REPO_ROOT / "content/experiences/index.ipynb"
...
def build_chart() -> go.Figure:
    """Return the index chart figure: overlaid career-signal KDE + concept cloud."""
    events = _collect_events()
    cloud_image = _build_word_cloud_image()
    ...
    fig.update_xaxes(
        ...
        title_font=dict(size=11, color="#bfbfbf"),
        tickfont=dict(size=10, color="#cfcfcf"),
        showgrid=True,
    ...
    fig.update_layout(
        ...
        font=dict(family="Helvetica, Arial, sans-serif", color="#e5e5e5", size=11),
        margin=dict(l=58, r=42, t=88, b=32),
        showlegend=False,
        hoverlabel=dict(
            bgcolor="#1b1b1b",
            bordercolor="#333333",
...
def _html_output(fig: go.Figure) -> dict[str, Any]:
    ...
    html = f'<div style="max-width:1120px;margin:0 auto">{inner}</div>'
    return {
...
def build_index_notebook() -> None:
    ...
    roles_cell = next(
        ...
        and "".join(cell.get("source", [])).lstrip().startswith("## Roles")
    )
